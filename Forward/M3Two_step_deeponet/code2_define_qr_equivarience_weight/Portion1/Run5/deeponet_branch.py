...
def load_model(filename):
    with open(filename, 'rb') as file:
        param1=pickle.load(file)
        param2=pickle.load(file)
...
paramst_batch1, paramst_batch2 = load_model(filename)
#print(paramst_batch1)
#print(paramst_batch2)
...
for epoch in range(NUM_EPOCHS):
    ...
    for i in range(0, phase_train.shape[0], BATCH_SIZE):
        if i == 0:
            phi1, phi2, phi3, Am1, Am2, Am3 = predict1(paramst_batch1,[phase_train[i:i+BATCH_SIZE], strain_2])
        else:
            phi1, phi2, phi3, Am1, Am2, Am3 = predict1(paramst_batch2,[phase_train[i:i+BATCH_SIZE], strain_2])
        Q1,R1 = torch.qr(phi1)
        Q2,R2 = torch.qr(phi2)
        Q3,R3 = torch.qr(phi3)
        ...
        params, loss_batch = update(params, phase_train[i:i+BATCH_SIZE], u_train)
