...
def plot_curves():
    ...
    negative_number2=stress_test_pred[0,0,1,30]
    stress_test = np.where(stress_test==negative_number, np.nan, stress_test)
    stress_test_pred = np.where(stress_weight_test==0.0, np.nan, stress_test_pred)
    ...
    for iplot, idx_plot in enumerate(plot_idx_list):
        ...
        def single_subplot(y, y_pred, irow, icol, title):
            ...
            strain_pred_extended, y_pred_extended = strain, y_pred
            strain_extended, y_extended = strain, y
            istage = 0
    ...
    plt.savefig(os.path.join(directory,'Fig_forward_curve.pdf'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(directory,'Fig_forward_parameters.pdf'))
    plt.close()
